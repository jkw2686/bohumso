...
test('No expiry reassignment, slot conflict, followup no-charge, refund isolation and stale revisions',async()=>{
 ...
 await login(ids.planner);assert.equal((await rpc('consultation_workspace',['partner'])).free_reserved,1);
 ...
 await login(ids.planner);await cmd('followup_confirm',{id:first,revision:(await row(first,'partner')).revision,followup_id:booking.followups[0].id});assert.equal((await row(first,'partner')).orders.length,0);assert.equal((await rpc('consultation_workspace',['partner'])).free_used,1);
 await login(ids.customer);await cmd('issue',{id:first,category:'dispute',reason:'가상 이의 제기'});await login(ids.planner);await assert.rejects(()=>rpc('connection_checkout',[first,2]),/free_no_payment/);
