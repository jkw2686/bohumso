...
export async function fixture(){
 ...
 for(const file of ['001_accounts.sql','002_requests.sql','003_consultations.sql','004_payment_ledger.sql','005_matching_worker.sql','006_metrics.sql','007_partner_onboarding.sql','009_planner_availability.sql']){try{await db.exec(await readFile('supabase/'+file,'utf8'));}catch(error){await db.close();throw Error(file+': '+error.message+' at '+error.position);}}
