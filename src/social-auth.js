...
export async function configureSocialAuth(config,mode){
 ...
 for(const {button} of items){button.disabled=true;button.textContent=button.dataset.comingSoon==='true'?'카카오 로그인 준비 중':'연결 확인 중';}
 ...
 for(const {id,label,button} of items){if(button.dataset.comingSoon==='true'){button.disabled=true;button.textContent=label+' 로그인 준비 중';continue;}const enabled=settingsLoaded&&(id==='naver'?config.naverLogin===true:external[id]===true);button.disabled=!enabled;button.textContent=enabled?label+(mode==='signup'?'로 시작하기':'로 로그인'):label+(settingsLoaded?' 로그인 준비 중':' 연결 확인 실패');}
