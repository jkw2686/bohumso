...
export const states={requested:'요청',coordinating:'일정 조율',confirmed:'양측 확정',scheduled:'미팅 예정',awaiting_completion:'완료 확인 대기',completed:'완료',cancelled:'취소',no_show:'노쇼 신고',dispute:'분쟁',unmatched:'요청 종료 · 직접 다시 선택'};
...
export function sortedProfiles(profiles,position,wanted=""){return [...profiles].map(p=>({...p,distance:distance(position,p)})).sort((a,b)=>(a.distance??Infinity)-(b.distance??Infinity)||(b.rating??-1)-(a.rating??-1)||Number(b.specialties?.includes(wanted))-Number(a.specialties?.includes(wanted))||a.name.localeCompare(b.name,'ko'));}
