...
## 보험소 PRO 청구준비 — 사용자 정정 반영
이 업무의 현행 기준은 [PRO 청구준비 Workflow](docs/pro-claim-preparation-workflow.md)입니다. 상담 후 필요 확인 → 고객·병원·서류 선택 → 고객에게 Secure Link 준비 요청 → 최소정보 확인·신분증 촬영·마스킹 → 사본 준비 완료 → 설계사 화면 반영 순서입니다. 위임장·동의서 사전 출력/지참을 전제로 하지 않습니다. 자필서명·종이 원본은 독립된 후속 상태로 관리하며 전자위임장 완료를 필수 조건으로 두지 않습니다. 고객에게 설계사가 입력한 병원·서류·청구상황을 다시 입력시키지 않습니다. 이 정정은 기존 결제·상담 기능의 변경이나 공개 배포를 허용하는 지시가 아닙니다.
