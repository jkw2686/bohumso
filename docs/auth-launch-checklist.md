...
## 2026-09-22 간편가입 후속 점검
- 공개 인증 설정 재조회: 이메일 활성, Kakao/Google 비활성. OAuth 실연결 완료 아님.
- 로컬 가입·로그인 화면에는 디자인 미리보기임을 명시하고 공개 사이트 가입·로그인 링크 추가. 로컬에서 실제 인증을 가장하지 않음.
- 간편로그인 공급자 비활성과 조회 실패를 구분. 일시 오류 시 다시 확인 버튼 제공. 가입 회귀/모바일/PKCE 모의 테스트 통과.
- Google Cloud 약관은 사용자가 직접 완료. 현재 계정 2단계 인증 요구로 Cloud 접근 차단. 열린 Google 본인 확인 화면에서 사용자가 완료해야 후속 설정 가능.
- 카카오 Developers는 로그인 화면. 사용자가 로그인해야 앱/콜백 설정을 이어갈 수 있음.
- 009 DB 상태 기능 실제 적용 및 브랜드/인증 UI 공개 배포는 아직 수행하지 않음. 배포 승인 전 준비 상태 유지.
