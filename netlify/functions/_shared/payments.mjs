export function testPaymentConfig(env){
 ...
 if(env('PAYMENTS_ENABLED')!=='true'||env('TOSS_MODE')!=='test'||!clientKey.startsWith('test_gck_')||!secretKey.startsWith('test_gsk_'))throw new Error('test_payment_not_configured');
...
export function createTossGateway(secretKey,fetcher=fetch){
 if(!secretKey?.startsWith('test_gsk_'))throw Error('live_payments_blocked');
...
export function verifiedPayment(payment,order){
 if(payment.orderId!==order.id||payment.paymentKey!==order.payment_key||payment.totalAmount!==order.amount||payment.currency!=='KRW'||payment.type!=='NORMAL'||(['DONE','CANCELED','PARTIAL_CANCELED'].includes(payment.status)&&(!['카드','간편결제'].includes(payment.method)||(payment.method==='카드'&&!payment.card)||(payment.method==='간편결제'&&!payment.easyPay))))throw Error('provider_payment_mismatch');
