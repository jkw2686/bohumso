...
export default async (request:Request)=>{
 ...
 try{
  ...
  if(typeof id!=='string'||!/^boh_[a-f0-9]{32}_[12]$/.test(id))return new Response(null,{status:200});
  ...
  const {data:order,error}=await client.rpc('connection_order_lookup',{order_id:id});if(error)throw Error('lookup');if(!order?.payment_key)return new Response(null,{status:200});
  ...
  const result=await client.rpc('connection_reconcile',verifiedPayment(payment,order));if(result.error)throw Error('reconcile');
