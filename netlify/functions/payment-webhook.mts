...
export default async (request:Request)=>{
 ...
 try{
  ...
  const {data:order,error}=await client.rpc('ad_order_lookup',{order_id:id});if(error)throw Error('lookup');if(!order?.payment_key)return new Response(null,{status:200});
  ...
  const result=await client.rpc('ad_reconcile',verifiedPayment(payment,order));if(result.error)throw Error('reconcile');
